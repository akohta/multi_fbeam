...
void calc_bsb_EH_dv(double complex *e,double complex *h,double complex *dedv,double complex *dhdv,double *x,double *v,Bsb *bsb)
{
  ...
  calc_bsb_eh_dv(te,th,tde,tdh,xn,nn,bsb);
...
void calc_bsb_eh_dv(double complex *e,double complex *h,double complex *dedv,double complex *dhdv,double *x,double *v,Bsb *bsb)
{
  void int_phi_bsb_dv(double complex *eh,double complex *deh,double ct,double st,double *x,double *v,Bsb *bsb);
  ...
  ct=bsb->data.ct;
  st=bsb->data.st;
  int_phi_bsb_dv(eh,deh,ct,st,x,v,bsb);
...
void int_phi_bsb_dv(double complex *eh,double complex *deh,double ct,double st,double *x,double *v,Bsb *bsb)
{
  void ac_eh_bsb_dv(double complex *eh,double complex *deh,double cos_t,double sin_t,double cos_p,double sin_p,double *x,double *v,Bsb *bsb);
  ...
  cc=0;
  h=2.0*M_PI;
  ac_eh_bsb_dv(Ia,dIa,ct,st,bsb->data.cp[0],bsb->data.sp[0],x,v,bsb);
  abst=0.0;
  for(i=0;i<6;i++){
    It[i]=h*Ia[i];
    dIt[i]=h*dIa[i];
  ...
  while(cc<=TRAP_MNUM){
    ...
    for(s=1;s<=sc;s++){
      ac_eh_bsb_dv(Ia,dIa,ct,st,bsb->data.cp[j],bsb->data.sp[j],x,v,bsb);
      j++;
      for(i=0;i<6;i++){
    	  Is[i]+=Ia[i];
    	  dIs[i]+=dIa[i];
