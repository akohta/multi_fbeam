...
void read_data_lgb(char *rfile,LGb *lgb)
{
  ...
  fscanf(fp,"%d",&tmpi); 
  if(tmpi!=2){ printf("beam type %d is not supported\n",tmpi); exit(1);}
  else                                                printf("incident beam type                          : Lagueere Gaussian beam\n");
  fscanf(fp,"%lf",&tmpd);  lgb->lambda0=tmpd;         printf("wave length of incident beam in vacuum   [m]: %15.14g\n",tmpd);
  fscanf(fp,"%lf",&tmpd);  lgb->ni     =tmpd;         printf("refractive index of surrounding             : %15.14g\n",tmpd); 
  fscanf(fp,"%lf",&tmpd);  lgb->NA     =tmpd;         printf("numerical aperture                          : %15.14g\n",tmpd);
  fscanf(fp,"%lf",&tmpd);  lgb->power  =tmpd;         printf("incident beam power                      [W]: %15.14g\n",tmpd);
  fscanf(fp,"%lf",&tmpd); 
  fscanf(fp,"%lf",&tmpd2); lgb->e0x    =tmpd+I*tmpd2; printf("x-component of polarization coefficient     :%7.6g+%7.6gi\n",tmpd,tmpd2);
  fscanf(fp,"%lf",&tmpd);
  fscanf(fp,"%lf",&tmpd2); lgb->e0y    =tmpd+I*tmpd2; printf("y-component of polarization coefficient     :%7.6g+%7.6gi\n",tmpd,tmpd2);
  fscanf(fp,"%lf",&tmpd);  lgb->fx     =tmpd;         printf("x-component of translation vector        [m]: %15.14g\n",tmpd);
  fscanf(fp,"%lf",&tmpd);  lgb->fy     =tmpd;         printf("y-component of translation vector        [m]: %15.14g\n",tmpd);
  fscanf(fp,"%lf",&tmpd);  lgb->fz     =tmpd;         printf("z-component of translation vector        [m]: %15.14g\n",tmpd);
  fscanf(fp,"%lf",&tmpd);  lgb->theta  =tmpd;         printf("rotation parameter theta               [rad]: %15.14g\n",tmpd);
  fscanf(fp,"%lf",&tmpd);  lgb->phi    =tmpd;         printf("rotation parameter phi                 [rad]: %15.14g\n",tmpd);
  fscanf(fp,"%d" ,&tmpi);  lgb->lg_m   =tmpi;         printf("mode number for spiral phase modulation     : %15d\n",tmpi);
  fscanf(fp,"%d" ,&tmpi);  lgb->nn     =tmpi;         printf("gauss-legendre integration sampling number  : %15d\n",tmpi);
...
void setup_LGb(LGb *lgb)
{
  ...
  lgb->data.hx=-lgb->data.ey*lgb->ni/Z0;
  lgb->data.hy= lgb->data.ex*lgb->ni/Z0;
  lgb->data.E0=sqrt(2.0*Z0*lgb->power*lgb->ni)/lgb->lambda0;
  lgb->data.cos_t=cos(lgb->theta);  lgb->data.sin_t=sin(lgb->theta);
  lgb->data.cos_p=cos(lgb->phi);    lgb->data.sin_p=sin(lgb->phi);
  lgb->data.nt=lgb->nn;
  // gau-leg data 
  lgb->data.ct=(double *)malloc(sizeof(double)*lgb->data.nt);
  lgb->data.st=(double *)malloc(sizeof(double)*lgb->data.nt);
  lgb->data.wt=(double *)malloc(sizeof(double)*lgb->data.nt);
  xt=(double *)malloc(sizeof(double)*lgb->data.nt);
  ...
  // trapezoid data
  nnc=(2<<(TRAP_MNUM))+1;
  lgb->data.cp=(double *)malloc(sizeof(double)*nnc);
  lgb->data.sp=(double *)malloc(sizeof(double)*nnc);
  lgb->data.ph_p=(double complex *)malloc(sizeof(double complex)*nnc);
  h=2.0*M_PI;
  lgb->data.cp[0]=-1.0;
  lgb->data.sp[0]= 0.0;
...
void int_phi_lgb(double _Complex *eh,double ct,double st,double *x,LGb *lgb)
{
  ...
  double cep=lgb->ni*lgb->ni*epsilon0;
  double cmu=mu0;
